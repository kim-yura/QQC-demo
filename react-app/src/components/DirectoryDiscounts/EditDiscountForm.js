...
import { editDiscount, loadDiscounts } from '../../store/discount';
...
function EditDiscountForm() {
    ...
    const chosenDiscount = allDiscounts[parseInt(id)];

    const [name, setName] = useState(chosenDiscount ? chosenDiscount.name : '');
    const [website, setWebsite] = useState(chosenDiscount ? chosenDiscount.website : '');
    const [instagram, setInstagram] = useState(chosenDiscount ? chosenDiscount.instagram : '');
    const [country, setCountry] = useState(chosenDiscount ? chosenDiscount.country : '');
    const [isYarn, setIsYarn] = useState(chosenDiscount ? chosenDiscount.isYarn : false);
    const [isFiber, setIsFiber] = useState(chosenDiscount ? chosenDiscount.isFiber : false);
    const [isNotions, setIsNotions] = useState(chosenDiscount ? chosenDiscount.isNotions : false);
    const [isPattern, setIsPattern] = useState(chosenDiscount ? chosenDiscount.isPattern : false);
    const [isKnitting, setIsKnitting] = useState(chosenDiscount ? chosenDiscount.isKnitting : false);
    const [isCrochet, setIsCrochet] = useState(chosenDiscount ? chosenDiscount.isCrochet : false);
    const [isEmbroidery, setIsEmbroidery] = useState(chosenDiscount ? chosenDiscount.isEmbroidery : false);
    const [discountCode, setDiscountCode] = useState(chosenDiscount ? chosenDiscount.discountCode : '');
    const [discountDetails, setDiscountDetails] = useState(chosenDiscount ? chosenDiscount.discountDetails : '');
    ...
    const handleSubmit = async (e) => {
        ...
        if (!discountCode) errors.push('Discount needs a code.');
        if (!discountDetails) errors.push('Discount needs details.');

        setValidationErrors(errors);

        if (!errors.length) {
            await dispatch(editDiscount(editedDiscount));
        ...
        <div className='site-body'>
            <div className='home-section'>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <p>Maker Country*</p>
                    <select defaultValue={country} id="country" name="country" onChange={(e) => setCountry(e.target.value)}>
